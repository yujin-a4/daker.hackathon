  ...
  return (
    <div>
      <section className="relative bg-background overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-50/50 to-violet-100/50 dark:from-indigo-950/50 dark:to-violet-950/50 opacity-50" />
        <div
        ...
        <div className="relative container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 text-center">
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
            <QuickCard
              icon={Trophy}
              title="참가 가능 대회"
              desc={`${recruitingHackathons.length}개 모집중`}
              link="/hackathons"
            />
            <QuickCard
              icon={Users}
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground font-medium max-w-md">
                  참여 중인 프로젝트의 진행 상황과 팀 협업 흐름을 바로 이어갈 수 있습니다.
                </p>
              </div>
            </div>
                    ...
                    <div className="mb-6">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
                        {team.progressStatus === 'developing'
                          ? '핵심 기능 구현과 제출 완성도를 끌어올리는 중입니다.'
                          : '기획 정리와 MVP 범위 확정을 진행 중입니다.'}
                      </p>
                    </div>
      ...
      <section className="py-16 lg:py-24 bg-muted/20">
        <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-12">
            <h2 className="text-3xl font-bold text-foreground tracking-tight">지금 참가 가능한 해커톤</h2>
          ...
          {recruitingHackathons.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recruitingHackathons.slice(0, 3).map((hackathon) => (
            ...
            <EmptyState
              icon={Boxes}
              title="현재 모집 중인 해커톤이 없습니다"
              description="새 모집이 열리면 여기서 바로 확인할 수 있습니다."
            />
          )}
        </div>
      </section>
