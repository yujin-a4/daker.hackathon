...
export default function RankingTable({ rankings }: RankingTableProps) {
  ...
  const PodiumItem = ({ user, position }: { user: RankingUser; position: number }) => {
    ...
    const heights = ['h-48 md:h-56', 'h-40 md:h-48', 'h-32 md:h-40'];
    const colors = [
      'bg-amber-400 dark:bg-amber-600',
      'bg-slate-300 dark:bg-slate-500',
      'bg-orange-400 dark:bg-orange-600'
    ];
    const order = [1, 0, 2]; // Left(2), Middle(1), Right(3)

    return (
      <div className={cn("flex flex-col items-center justify-end flex-1 max-w-[120px] md:max-w-[160px]")}>
        <div className="mb-3 text-center">
          <Avatar className={cn("h-14 w-14 md:h-20 md:w-20 border-4 shadow-lg", isCurrentUser ? "border-primary" : "border-white dark:border-slate-800")}>
            <AvatarFallback className="bg-muted text-lg font-bold">
              {getInitials(user.nickname)}
            </AvatarFallback>
          </Avatar>
          <div className="mt-2">
            <p className="text-sm md:text-base font-bold truncate max-w-[80px] md:max-w-[120px]">{user.nickname}</p>
        ...
        <motion.div
          initial={{ height: 0 }}
          animate={{ height: 'auto' }}
          className={cn("w-full rounded-t-xl flex flex-col items-center pt-2 md:pt-4 gap-1 shadow-inner", colors[position], position === 0 ? "h-32 md:h-44" : position === 1 ? "h-24 md:h-32" : "h-20 md:h-24")}
        >
