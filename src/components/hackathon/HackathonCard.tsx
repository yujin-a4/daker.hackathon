...
export default function HackathonCard({ hackathon }: HackathonCardProps) {
  ...
  return (
    <Card
      className="group relative flex flex-col h-full rounded-[10px] overflow-hidden transition-all duration-200 cursor-pointer border border-border bg-card hover:border-primary"
      ...
      <div className="p-5 flex flex-col h-full gap-4">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-start gap-4">
            <h4 className="text-base font-bold leading-tight text-foreground group-hover:text-primary transition-colors line-clamp-2">
              {hackathon.title}
            </h4>
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={handleBookmarkClick}
              className="flex-shrink-0 p-1.5 rounded-[4px] hover:bg-[#F8F7F4] text-[#6B6B6B] transition-colors"
            >
              <Heart className={cn("w-4 h-4", isBookmarked ? "fill-rose-500 text-rose-500 border-none" : "fill-transparent")} />
            </motion.button>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <Badge className={cn('font-medium text-[10px] uppercase tracking-wider rounded-[6px] px-2 py-0.5', 
              hackathon.status === 'ongoing' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground')}>
              {statusText}
            </Badge>
            <Badge variant="outline" className="text-[10px] font-normal border-border text-muted-foreground rounded-[6px]">
              {hackathon.type}
            </Badge>
            {hackathon.status !== 'ended' && (
              <div className={cn(
                'text-[10px] font-medium px-2 py-0.5 rounded-[6px] border',
                isDdayUrgent ? 'text-accent border-accent/30 bg-accent/10' : 'text-muted-foreground border-border'
              )}>
                {dday}
              </div>
            )}
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between text-[10px] font-medium uppercase tracking-widest text-muted-foreground">
            <span>Progress</span>
            <span className={cn("font-medium", isDdayUrgent ? "text-accent" : "text-foreground")}>{Math.round(progress)}%</span>
          </div>
          <div className="h-[2px] w-full bg-border rounded-full overflow-hidden">
            <div
              className={cn(
                "h-full transition-all duration-1000",
                hackathon.status === 'ended' ? "bg-muted-foreground" : isDdayUrgent ? "bg-accent" : "bg-primary"
              )}
