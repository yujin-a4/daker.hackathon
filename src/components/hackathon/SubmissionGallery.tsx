...
export default function SubmissionGallery({ entries, currentPhaseKey, isVotingPhase, onVote }: SubmissionGalleryProps) {
        ...
        <Card key={entry.teamCode} className={cn(
          ...
          <div className="aspect-video bg-slate-100 dark:bg-slate-800 relative flex items-center justify-center overflow-hidden">
            ...
            <div className="text-center group-hover:scale-110 transition-transform duration-500">
               {entry.artifacts.some(a => a.key === 'web') ? (
          ...
          <CardFooter className="p-5 pt-0 flex items-center justify-between border-t border-slate-50 dark:border-slate-800/50 mt-auto">
            ...
            <Button 
              ...
              disabled={!isVotingPhase || entry.isMyTeam}
              onClick={() => onVote?.(entry.teamCode)}
              className={cn(
                "h-8 font-bold text-xs rounded-lg px-4",
                isVotingPhase ? "bg-indigo-600 hover:bg-indigo-700" : ""
              )}
            >
              {isVotingPhase ? '투표하기' : '작품 보기'}
            </Button>
          </CardFooter>
        </Card>
      ))}
