...
import { CheckCircle, XCircle, Upload, FileText, Link as LinkIcon, AlertTriangle } from 'lucide-react';
...
export default function SubmitSection({ hackathonSlug, hackathonDetail }: SubmitSectionProps) {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      form.setValue('content', file.name); // for validation
  ...
  return (
    <div className="space-y-8">
        ...
        <ul className="space-y-4">
          {submitInfo.guide.map((item, index) => (
            <li key={index} className="flex items-start gap-3">
              ...
              <span className="text-blue-900 mt-0.5">{item}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      {submitInfo.submissionItems && (
        <div>
          <h3 className="text-lg font-bold text-slate-800 mb-4">제출 현황</h3>
      ...
      <div>
        <h3 className="text-lg font-bold text-slate-800 mb-4">새 제출</h3>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 p-6 bg-white border rounded-lg">
          {submitInfo.submissionItems && (
             <Controller
          ...
          <div>
            {(format === 'zip' || format === 'pdf' || format === 'pdf_url') && (
              <div className="border-dashed border-2 border-slate-300 rounded-xl p-8 text-center">
                ...
                <label htmlFor="file-upload" className="cursor-pointer">
                  ...
                  <p className="mt-2 text-sm text-slate-500">{selectedFile ? selectedFile.name : '파일을 드래그하거나 클릭하여 업로드'}</p>
                </label>
              </div>
            )}
            
            {format === 'text_or_url' && (
              // This is a simplified version, for a real app, use Tabs component
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700">제출하기</Button>
          </div>
        </form>
      </div>
    </div>
